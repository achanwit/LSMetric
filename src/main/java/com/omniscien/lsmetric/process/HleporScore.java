...
public class HleporScore {
	...
	public String getScoreFromTextFile(int CaseSensitive, String InputReference, String InputCandidate,
		...
		if (OutputFormat == 2 || OutputFormat == 3 || OutputFormat == 4) {
			if (OutputFormat == 2) {
				if (CaseSensitive == 1) {
					...
					resultSummaryModel.setMetric(Constant.METRIC_HLEPOR);
					...
					resultSummaryCSModel.setMetric(Constant.METRIC_HLEPOR);
					...
					resultSummaryCIModel.setMetric(Constant.METRIC_HLEPOR);
				}
				...
				if (CaseSensitive == 1) {
					...
					resultPerLineOnlyModel.setMetric(Constant.METRIC_HLEPOR);
					...
					resultPerLineOnlyCSModel.setMetric(Constant.METRIC_HLEPOR);
					...
					resultPerLineOnlyCIModel.setMetric(Constant.METRIC_HLEPOR);
		...
		try {
			...
			if(InputReferenceList.size() != InputCandidateList.size()) {
				...
				resultRespondOnly.metric = Constant.METRIC_HLEPOR;
	...
	private List<Double> getCsoreList(String rawScoreCSStr, List<Double> cs) {
		...
		String scoreEachLine = "";
		for(int i =0;i<resultArr.length;i++) {
			if(resultArr[i].trim().equals("evaluation value HLEPOR of every single sentence:")) {
				scoreEachLine = resultArr[i+1];
				break;
			}
		}
		String[] scoreArr = scoreEachLine.split(" ");
		for(int i = 0; i< scoreArr.length; i++) {
			String scoreRaw = scoreArr[i];
			double scoreDouble= Double.valueOf(scoreRaw);
			scoreDouble = scoreDouble*100;
			DecimalFormat df = new DecimalFormat("#.##");
			String scoreFormat = df.format(Double.valueOf(scoreDouble));
			scoreDouble = Double.valueOf(scoreFormat);
			cs.add(scoreDouble);
		}
		return cs;
	}
	...
	private double normalizeROUGEOutputSummary(String rawScoreCSStr) {
		...
		String Sumresult = "";
		for(int i = 0; i< resultArr.length; i++) {
			if(resultArr[i].trim().equals("mean value HLEPOR of all single sentence:")) {
				Sumresult = resultArr[i+1];
				break;
			}
		}
		double scoreSum = Double.valueOf(Sumresult);
		scoreSum = scoreSum*100;
		DecimalFormat df = new DecimalFormat("#.##");
		String scoreFormat = df.format(Double.valueOf(scoreSum));
		scoreSum = Double.valueOf(scoreFormat);
		return scoreSum;
	}

	private String getRawScore(String inputReference, String inputCandidate, boolean b) throws IOException, InterruptedException {
		String hleporScript = rp.getProp(Constant.hlepor_Script);
		...
		String[] commands = {hleporScript, refOption, inputReference, canOption, inputCandidate};
