	...
	public String getScoreFromTextFile(int CaseSensitive, String InputReference, String InputCandidate,
		...
		if (OutputFormat == 2 || OutputFormat == 3 || OutputFormat == 4) {
			if (OutputFormat == 2) {
				if (CaseSensitive == 1) {
					...
					resultSummaryModel.setMetric(Constant.METRIC_METEOR);
					...
					resultSummaryCSModel.setMetric(Constant.METRIC_METEOR);
					...
					resultSummaryCIModel.setMetric(Constant.METRIC_METEOR);
				}
				...
				if (CaseSensitive == 1) {
					...
					resultPerLineOnlyModel.setMetric(Constant.METRIC_METEOR);
					...
					resultPerLineOnlyCSModel.setMetric(Constant.METRIC_METEOR);
					...
					resultPerLineOnlyCIModel.setMetric(Constant.METRIC_METEOR);
		...
		try {
			...
			if(InputReferenceList.size() != InputCandidateList.size()) {
				...
				resultRespondOnly.metric = Constant.METRIC_METEOR;
...
//			System.out.println(linePieces[1]);
			String scoreStr = linePieces[1];
			
			double finalScoreDouble = Double.valueOf(scoreStr);
			finalScoreDouble = finalScoreDouble*100;
			scoreStr = String.valueOf(finalScoreDouble);
			DecimalFormat df = new DecimalFormat("#.##");
			String scoreFormat = df.format(Double.valueOf(scoreStr));
			finalScoreDouble = Double.valueOf(scoreFormat);
			cs.add(finalScoreDouble);
		}
		return cs;
	}
	...
	private double normalizeROUGEOutputSummary(String rawResult) {
...
//		System.out.println("total: "+total);
	...
	private String getRawScore(String inputReference, String inputCandidate, boolean cassencitive) throws IOException, InterruptedException {
		String javaStr1 = "java";
		String javaStr2 = "-jar";
		String meteorJar = rp.getProp(Constant.meteor_Jar);
