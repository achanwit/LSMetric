...
import com.ao.ter.TERalignment;
import com.ao.ter.TERcalc;

public class TranslationErrorScore {
		...
		if (OutputFormat == 2 || OutputFormat == 3 || OutputFormat == 4) {
			if (OutputFormat == 2) {
				if (CaseSensitive == 1) {
					...
					resultSummaryModel.setMetric(Constant.METRIC_TER);
					...
					resultSummaryCSModel.setMetric(Constant.METRIC_TER);
					...
					resultSummaryCIModel.setMetric(Constant.METRIC_TER);
				}
				...
				if (CaseSensitive == 1) {
					...
					resultPerLineOnlyModel.setMetric(Constant.METRIC_TER);
					...
					resultPerLineOnlyCSModel.setMetric(Constant.METRIC_TER);
					...
					resultPerLineOnlyCIModel.setMetric(Constant.METRIC_TER);
	...
	private double getScore(String ref, String can, boolean caseSensitive) {
		double score = 0;
		TERcalc.setCase(caseSensitive);
		TERalignment ta = TERcalc.TER(can, ref);
		Double dScore = ta.score();
		if(dScore >= 1) {
			score =  100;
		}else {
			score = dScore * 100;
