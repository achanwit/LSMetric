...
public class RougeScore {
		...
		if (OutputFormat == 2 || OutputFormat == 3 || OutputFormat == 4) {
			if (OutputFormat == 2) {
				if (CaseSensitive == 1) {
					...
					resultSummaryModel.setMetric(Constant.METRIC_ROUGE);
					...
					resultSummaryCSModel.setMetric(Constant.METRIC_ROUGE);
					...
					resultSummaryCIModel.setMetric(Constant.METRIC_ROUGE);
				}
				...
				if (CaseSensitive == 1) {
					...
					resultPerLineOnlyModel.setMetric(Constant.METRIC_ROUGE);
					...
					resultPerLineOnlyCSModel.setMetric(Constant.METRIC_ROUGE);
					...
					resultPerLineOnlyCIModel.setMetric(Constant.METRIC_ROUGE);
			...
			try {
				...
				if(InputReferenceList.size() != InputCandidateList.size()) {
					...
					resultRespondOnly.metric = Constant.METRIC_ROUGE;
				...
				if (OutputFormat == 2) {
					if (CaseSensitive == 1) {
						double scoreSummaryCS = getScoreSummary(InputReference, InputCandidate, true);
						double scoreSummaryCI = getScoreSummary(InputReference, InputCandidate, false);
						...
						double scoreSummaryCI = getScoreSummary(InputReference, InputReference, false);
	...
	private List<Double> getCsoreList(String rawScoreCSStr, List<Double> cs) {
		...
		Matcher lineMatcher = linePattern.matcher(rawScoreCSStr);
		...
		while (lineMatcher.find()) {
			...
			double linePrecision = Double.parseDouble(linePieces[1]);
//			double lineRecall = Double.parseDouble(linePieces[2]);
//			double lineFScore = Double.parseDouble(linePieces[3]);
//			lineBuilder.append(lineCounter + "\t" + linePrecision + lineRecall + "\t" + lineFScore + "\n");
			cs.add(linePrecision);
			lineCounter++;
		}
	...
	private double getScoreSummary(String inputReference, String inputCandidate, boolean caseSensitive) throws Exception {
		double SummaryScore = 0;
		String rawStr = getRawScore(inputCandidate, inputReference, caseSensitive);
		SummaryScore = normalizeROUGEOutputSummary(rawStr);
		return SummaryScore;
	}



	private double normalizeROUGEOutputSummary(String summaryScoreStr) {
		double totalFScore = 0;
		String totalStr = "^Total_.*";

		Pattern totalPattern = Pattern.compile(totalStr, Pattern.MULTILINE);
		Matcher totalMatcher = totalPattern.matcher(summaryScoreStr);

		int lineCounter = 1;
		StringBuilder lineBuilder = new StringBuilder();

		while (totalMatcher.find()) {
			String total = totalMatcher.group(0);
			String[] totalPieces = total.split("\t");
			switch (totalPieces[0]) {
			case "Total_F_score:":
				totalFScore = Double.parseDouble(totalPieces[1]);
				break;
			}
		}
	...
	private String getRawScore(String candFile, String refFile, boolean caseInsensitive) throws Exception {
		...
		try {
			...
			String pythonFile = rp.getProp(Constant.rouge_python_file);
			String[] commands = { "python3", pythonFile, candFile, refFile, caseFlag };
			...
			rawOutput  =_inputStreamGobbler.getOutputBuffer();
