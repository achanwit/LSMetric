...
public class RibesScore {
	...
	public String getScoreFromTextFile(int CaseSensitive, String InputReference, String InputCandidate,
		...
		if (OutputFormat == 2 || OutputFormat == 3 || OutputFormat == 4) {
			if (OutputFormat == 2) {
				if (CaseSensitive == 1) {
					...
					resultSummaryModel.setMetric(Constant.METRIC_RIBES);
					...
					resultSummaryCSModel.setMetric(Constant.METRIC_RIBES);
					...
					resultSummaryCIModel.setMetric(Constant.METRIC_RIBES);
				}
				...
				if (CaseSensitive == 1) {
					...
					resultPerLineOnlyModel.setMetric(Constant.METRIC_RIBES);
					...
					resultPerLineOnlyCSModel.setMetric(Constant.METRIC_RIBES);
					...
					resultPerLineOnlyCIModel.setMetric(Constant.METRIC_RIBES);
		...
		try {
			...
			if(InputReferenceList.size() != InputCandidateList.size()) {
				...
				resultRespondOnly.metric = Constant.METRIC_RIBES;
	...
	private List<Double> getCsoreList(String rawScoreCSStr, List<Double> cs) {
		...
		for(int i =0;i<resultArr.length-1;i++) {
			String scoreSummmary = resultArr[i].substring(0,8);
			DecimalFormat df = new DecimalFormat("#.##");
			String scoreFormat = df.format(Double.valueOf(scoreSummmary));
			
			cs.add(Double.valueOf(scoreFormat));
		}
		return cs;
	}
	...
	private double normalizeROUGEOutputSummary(String rawScoreCSStr) {
		String[]  resultArr = rawScoreCSStr.split("\n");
		String scoreSummmary = resultArr[resultArr.length-1].substring(0,8);
		DecimalFormat df = new DecimalFormat("#.##");
		String scoreFormat = df.format(Double.valueOf(scoreSummmary));
		
		return Double.valueOf(scoreFormat);
	}

	private String getRawScore(String inputReference, String inputCandidate, boolean caseSensitive) throws IOException, InterruptedException {
		String python3 = "python3";
		String RibseScript = rp.getProp(Constant.ribse_Script);
		String referArg = "-r";
		String s = "-s";
		String c = "-c";
		...
		String[] commands = {python3, RibseScript,  referArg, inputReference, inputCandidate, s, c};
